...
# from rpki_historical import validate
...
def have_reserved_asn(path):
    ...
    for i in path:
        ...
        if i == 0 \
                ...
                or (i >= 4200000000 and i <= 4294967295):
            ret.append(str(i))
    if ret:
        return ";".join(set(ret))
    return "-"
...
def origin_rpki_valid(prefix, path, timestamp):
    return rpki_valid(prefix, path[-1])
    # return validate(prefix, path[-1], timestamp)
...
@click.command()
@click.option("--collector", "-c", type=str, default="all_collectors", help="the name of RouteView collector to postprocess the detection results")
...
def postprocess(collector, year, month, day, hour, minute, model, dimension):
    ...
    flags_dir = reported_alarm_dir.parent/f"{target_date}_{dimension}.flags"
    flags_dir.mkdir(parents=True, exist_ok=True)

    print(f"Save flags to {flags_dir}")
    ...
    for i in tqdm(info):
        ...
        path1 = [i.split(" ") for i in df["diff_path_1"].values]
        path2 = [i.split(" ") for i in df["diff_path_2"].values]
        
        # path1 = [list(map(get_one_asn, i.split(" "))) for i in df["diff_path_1"].values]
        # path2 = [list(map(get_one_asn, i.split(" "))) for i in df["diff_path_2"].values]
        ...
        flag_file = flags_dir/f"{Path(i['save_path']).stem}.flags.csv"

        if flag_file.exists(): 
            flag = pd.read_csv(flag_file, quotechar='"')
            if len(flag) == len(df):
                # print(f"skip {i['save_path']}")
                continue
            
            try:
                flag_file.unlink()
            except Exception as e:
                print(f"⚠️ Failed to delete {flag_file}: {e}")
                pass
            print(f"Re-create {flag_file}")
        ...
        flags = pd.DataFrame.from_dict({
            ...
            "origin_rpki_1": [origin_rpki_valid(p, l, ts) for p, l, ts in zip(prefix1, path1, timestamps)],
            "origin_rpki_2": [origin_rpki_valid(p, l, ts) for p, l, ts in zip(prefix2, path2, timestamps)],
        ...
        flags.to_csv(flags_dir/f"{Path(i['save_path']).stem}.flags.csv", index=False, quoting=csv.QUOTE_NONNUMERIC)
